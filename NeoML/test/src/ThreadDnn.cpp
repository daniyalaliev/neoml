...
IThreadPool::TFunction RunDnn = [](int, void* params)
{
...
TEST(ThreadTest, Four1Threads)
{
    ...
    for(int i = 0; i < numOfThreads; ++i) {
        ...
        for(int j = 0; j < blobs[i]->GetDataSize(); ++j) {
            blobs[i]->GetData().SetValueAt(j, static_cast<float>(CRandom(0).Uniform(-1, 1)));
        }
...
TEST(ThreadTest, Four1Dnns)
{
    ...
    for(int i = 0; i < numOfThreads; ++i) {
        ...
        for(int j = 0; j < blobs[i]->GetDataSize(); ++j) {
            blobs[i]->GetData().SetValueAt(j, static_cast<float>(randoms[i].Uniform(-1, 1)));
        }
